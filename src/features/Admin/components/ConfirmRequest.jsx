...
export default function ConfirmRequest({ order, onClose }) {
    const [loading, setLoading] = useState(false)
    const { updateRequest } = useAdminContext()

    const handleApprove = () => {
        updateRequest(order.id)
        onClose()
    }
    ...
    return (
        <>
            ...
            <div className='flex w-full justify-center py-[2rem] gap-[5rem]'>
                ...
                <Button background={"green"} onClick={handleApprove} >
                    confirm
                </Button>
            </div>
